# ==========================================
# Codigo 3: aprendizaje máquina con la particularidad de separar los datos en singulares de grupos/parejas.
...
# ==============================================================================
# 6. LIMPIEZA, SEPARACIÓN ALEATORIA Y CONSTRUCCIÓN DE MATRICES MATEMÁTICAS (X, Y)
# ==============================================================================
print("\n" + "="*50)
print(" PREPARACIÓN DE MATRICES (TRAIN vs TEST GLOBAL 95/5) ")
print("="*50)
...
# === ESTRATEGIA DE VALIDACIÓN CONJUNTA (95% Train / 5% Test) ===
# Juntamos los datos de TODAS las ciudades para hacer el análisis global

if dfs_ciudades:
    print(f"🎯 Variables objetivo configuradas: ISOPLEASANT e ISOEVENTFUL")
    
    # 1. Unimos todos los dataframes de las 4 ciudades en uno solo
    df_total = pd.concat(dfs_ciudades.values(), ignore_index=True)
    
    # 2. Aplicamos la función de limpieza a todo el conjunto global
    X_total, y_total_p, y_total_e, mask_total_grupos = aislar_variables_ml(df_total)
    
    # 3. Separamos aleatoriamente en 95% Train y 5% Test
    # Pasamos las X, las dos Y y la máscara de grupos para que se dividan de forma síncrona
    (X_train, X_test, 
     y_train_p, y_test_p, 
     y_train_e, y_test_e, 
     mask_train_grupos, mask_test_grupos) = train_test_split(
        X_total, y_total_p, y_total_e, mask_total_grupos, 
        test_size=0.057, 
        random_state=42 # Fijamos la semilla para que sea reproducible
    )
    
    print(f"📚 SET DE ENTRENAMIENTO (Train 95%): {len(X_train)} encuestas (Todas las ciudades).")
    print(f"   -> Variables predictoras (Columnas X resultantes): {len(X_train.columns)}")
    print(f"📝 SET DE EXAMEN (Test 5%): {len(X_test)} encuestas (Todas las ciudades).")
else:
    raise ValueError("CRÍTICO: El diccionario dfs_ciudades está vacío o no se ha cargado correctamente.")
...
def generar_tabla_resumen_global(df_singles, df_grupos, target_name):
    ...
    carpeta_resultados = ruta_script / 'Graficas_IA_AM1_TFG' / 'Resultados_Metricas'
...
def generar_auditoria_individual_completa(mejores_modelos, X_train, y_train, X_test, y_test, mask_train, mask_test, target_name):
    ...
    carpeta_auditorias = ruta_script / 'Graficas_IA_AM1_TFG' / 'Auditorias_Individuales'
...
def generar_comparativa_global_ias(df_resumen_global, target_name):
    """
    Genera una gran gráfica de barras agrupando las métricas clave de todos 
    los modelos para facilitar la comparativa visual global.
    Utiliza el DataFrame global que ya contiene métricas de Singles y Grupos.
    """
    print(f"\n" + "🌟"*20)
    ...
    # Magia de Pandas: 'melt' convierte la tabla ancha en una tabla larga. 
    # Básicamente aplasta las columnas de métricas en filas, para que Seaborn pueda dibujar 
    # barras agrupadas (hue) por modelo de forma automática.
    ...
    # Mejoramos la leyenda para que no tape las barras
    plt.legend(title='Métricas de Evaluación', loc='upper right', framealpha=0.9, fontsize=11)
    
    # 4. AÑADIR NÚMEROS A LAS BARRAS
    for container in ax.containers:
        # Rotamos el texto 90 grados para que no se pisen si las barras son muy estrechas
        ax.bar_label(container, fmt='%.3f', padding=5, fontsize=10, rotation=90)
        
    # 5. AJUSTE DE EJES
    # Subimos un poco el techo de la gráfica para que los números rotados no se salgan del cuadro
    ...
    # 6. GUARDAR LA IMAGEN
    carpeta_comparativas = ruta_script / 'Graficas_IA_AM1_TFG' / 'Comparativas_Globales'
...
def extraer_e_imprimir_importancias(mejores_modelos, X_train, y_train, target_name):
    ...
    carpeta_pesos = ruta_script / 'Graficas_IA_AM1_TFG' / 'Interpretabilidad'
...
def generar_super_prediccion_ensamblada(mejores_modelos, df_rank_singles, X_test, y_test_real, target_name):
    ...
    plt.tight_layout()
    carpeta_ensamble = ruta_script / 'Graficas_IA_AM1_TFG' / 'Ensamblaje'
