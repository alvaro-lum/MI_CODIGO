# ==========================================
# Codigo 5: aprendizaje máquina con la particularidad de separar los datos en singulares de grupos/parejas.
# Train --> Londres 95%. Test --> Londres 5%.
# Estratificación en la locationid.
# ==========================================
import pandas as pd
import numpy as np
...
# === ESTRATEGIA DE VALIDACIÓN ESTRATIFICADA (LONDRES 95/5) ===
print(f"🎯 Variables objetivo configuradas: ISOPLEASANT e ISOEVENTFUL")

# Extraemos el DataFrame de Londres crudo (que aún conserva 'locationid')
df_londres_crudo = dfs_ciudades['Londres'].copy()

# Hacemos la partición 95% Train / 5% Test estratificando por 'locationid'
# Esto garantiza que el 5% de test tenga una proporción idéntica de todas las plazas/parques.
df_train_crudo, df_test_crudo = train_test_split(
    df_londres_crudo, 
    test_size=0.057, 
    random_state=42, 
    stratify=df_londres_crudo['locationid']
)

# Ahora aplicamos tu función de limpieza para separar X e Y en ambos sets.
# (Aquí es donde 'locationid' y las demás columnas prohibidas se borrarán automáticamente)
...
print(f"📚 SET DE ENTRENAMIENTO (Train): {len(X_train)} encuestas (Londres - 95%).")
print(f"   -> Variables predictoras (Columnas X resultantes): {len(X_train.columns)}")
print(f"📝 SET DE EXAMEN (Test): {len(X_test)} encuestas (Londres - 5.7% Estratificado).")# === ESTRATEGIA DE VALIDACIÓN ESTRATIFICADA (LONDRES 95/5) ===
...
def generar_tabla_resumen_global(df_singles, df_grupos, target_name):
    ...
    carpeta_resultados = ruta_script / 'Graficas_IA_Ldn1_TFG' / 'Resultados_Metricas'
...
def generar_auditoria_individual_completa(mejores_modelos, X_train, y_train, X_test, y_test, mask_train, mask_test, target_name):
    ...
    carpeta_auditorias = ruta_script / 'Graficas_IA_Ldn1_TFG' / 'Auditorias_Individuales'
...
def generar_comparativa_global_ias(df_resumen_global, target_name):
    ...
    # 6. GUARDAR LA IMAGEN
    carpeta_comparativas = ruta_script / 'Graficas_IA_Ldn1_TFG' / 'Comparativas_Globales'
...
def extraer_e_imprimir_importancias(mejores_modelos, X_train, y_train, target_name):
    ...
    carpeta_pesos = ruta_script / 'Graficas_IA_Ldn1_TFG' / 'Interpretabilidad'
...
def generar_super_prediccion_ensamblada(mejores_modelos, df_rank_singles, X_test, y_test_real, target_name):
    ...
    plt.tight_layout()
    carpeta_ensamble = ruta_script / 'Graficas_IA_Ldn1_TFG' / 'Ensamblaje'
