# ==========================================
# Codigo 6: aprendizaje máquina general sin distinción de grupos.
...
# ==============================================================================
# 6. LIMPIEZA Y SEPARACIÓN ESPACIAL (UNIVERSAL)
# ==============================================================================
print("\n" + "="*50)
...
# Esto asegura que todas las plazas (CamdenTown, RussellSq, etc.) estén bien repartidas.
...
# Aplicamos la función de limpieza a ambos sets (aquí es donde se borra 'locationid' y el texto)
X_train, y_train_p, y_train_e = aislar_variables_ml(df_train_crudo)
X_test,  y_test_p,  y_test_e  = aislar_variables_ml(df_test_crudo)

print(f"📚 SET DE ENTRENAMIENTO (Train): {len(X_train)} encuestas (Londres - 95%).")
print(f"📝 SET DE EXAMEN (Test): {len(X_test)} encuestas (Londres - 5% Estratificado).")
...
def evaluar_ciudad_completa(mejores_modelos, X_test, y_test_real, target_name):
    ...
    carpeta_resultados = ruta_script / 'Graficas_IA_Ldn2_TFG' / 'Resultados_Metricas'
...
def generar_auditoria_individual(mejores_modelos, X_train, y_train, X_test, y_test, target_name):
    ...
    carpeta_auditoria = ruta_script / 'Graficas_IA_Ldn2_TFG' / 'Auditorias_Individuales'
...
def generar_comparativa_global_ias(df_resumen_global, target_name):
    ...
    # Guardar la imagen
    carpeta_comparativas = ruta_script / 'Graficas_IA_Ldn2_TFG' / 'Comparativas_Globales'
...
def extraer_e_imprimir_importancias(mejores_modelos, X_train, y_train, target_name):
    ...
    carpeta_pesos = ruta_script / 'Graficas_IA_Ldn2_TFG' / 'Interpretabilidad'
...
def generar_super_prediccion_ensamblada(mejores_modelos, df_ranking_universal, X_test, y_test_real, target_name):
    ...
    plt.tight_layout()
    carpeta_ensamble = ruta_script / 'Graficas_IA_Ldn2_TFG' / 'Ensamblaje'
