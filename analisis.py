...
# Definimos la carpeta principal de gráficas de forma GLOBAL
carpeta_graficas_base = ruta_script / 'graficas_TFG'
...
try:
    ...
    # =====================================================================
    # --- Eliminar localizaciones de China ---
    # =====================================================================
    localizaciones_china = [
        'DadongSquare', 'LianhuashanParkEntrance', 'LianhuashanParkForest', 
        'OlympicSquare', 'PingshanPark', 'PingshanStreet', 
        'ZhongshanPark', 'ZhongshanSquare'
    ]
    
    if 'locationid' in df_datos.columns:
        # Convertimos la lista y la columna a minúsculas para que el filtrado sea infalible
        locs_china_lower = [loc.lower() for loc in localizaciones_china]
        
        # Nos quedamos SÓLO con las filas cuya localización NO esté en la lista de China
        df_datos = df_datos[~df_datos['locationid'].astype(str).str.lower().isin(locs_china_lower)]
        
        filas_sin_china = df_datos.shape[0]
        eliminadas_china = filas_originales - filas_sin_china
        print(f"🚫 EXCLUSIÓN: Se han eliminado {eliminadas_china} encuestas correspondientes a las localizaciones de China.")
    # =====================================================================
    ...
    # --- CAMBIO CLAVE: Triple validación para buscar duplicados reales ---
    if all(col in df_datos.columns for col in ['recordid', 'locationid', 'groupid']):
        df_datos = df_datos.drop_duplicates(subset=['recordid', 'locationid', 'groupid'])
    elif 'recordid' in df_datos.columns and 'locationid' in df_datos.columns:
        df_datos = df_datos.drop_duplicates(subset=['recordid', 'locationid'])
    else:
    ...
    ruta_exportacion = carpeta_graficas_base / "ISD_Datos_Limpios_TFG.csv"
...
def obtener_carpeta_guardado(localizacion="General"):
    """Crea y devuelve la ruta de una subcarpeta dentro de graficas_TFG."""
...
def resumir_y_exportar_ISO():
    ...
    ruta_exportacion = carpeta_graficas_base / "ISD_Parametros_ISO_TFG.csv"
...
def generar_radar_circunflejo_paq_global():
    ...
    def dibujar_radar(lista_datos, lista_colores, lista_etiquetas, titulo, nombre_archivo):
        ...
        for datos, color, etiqueta in zip(lista_datos, lista_colores, lista_etiquetas):
            ...
            ax.fill(angulos, datos, color=color, alpha=0.2) # Alpha más suave para ver cruces
        ...
        plt.close() 
        print(f"  ✅ Guardado: graficas_TFG/General/{ruta_img.name}")

    # --- 3. GENERACIÓN DE LAS 3 GRÁFICAS ---
    
    # Gráfica A: Solo Medias (Azul)
    ...
    # Gráfica B: Solo Medianas (Naranja/Rojo)
    ...
    # Gráfica C: Combinada (Medias vs Medianas)
...
def generar_radar_localizacion_especifica():
    ...
    # Sub-función interna para no repetir código de dibujo 3 veces
    ...
    for loc in locs_a_procesar:
        ...
        print(f"✅ 3 Gráficos guardados para {loc} en la carpeta correspondiente.")
...
def generar_iso_plot_combinado():
    ...
    for loc in locs_a_procesar:
        ...
        try:
            ...
            print(f"✅ Gráfico combinado guardado: graficas_TFG/{loc}/{ruta_img.name}")
        ...
        for loc in locs_a_procesar:
            try:
                ...
                if respuesta_grafica == 's':
                    ...
                    plt.savefig(ruta_img, bbox_inches='tight', dpi=300, facecolor='white')
                    plt.close()
                    print(f"✅ Gráfica comparativa SPI guardada: graficas_TFG/{loc}/{ruta_img.name}\n")
                else:
                ...
                if respuesta_superpuesta == 's':
                    ...
                    plt.close()
                    print(f"✅ Gráfica superpuesta guardada: graficas_TFG/{loc}/{ruta_img_super.name}\n")
                else:
...
def generar_scatter_agrupado_localizacion():
    ...
    try:
        ...
        plt.savefig(ruta_img, bbox_inches='tight', dpi=300, facecolor='white')
        plt.close()
        print(f"✅ Gráfico scatter agrupado guardado: graficas_TFG/General/{ruta_img.name}")
    ...
    while True:
        ...
        for loc in locs_a_procesar:
            ...
            try:
                ...
                plt.savefig(ruta_img, bbox_inches='tight', dpi=300, facecolor='white')
                plt.close()
                print(f"  ✅ Gráfico agrupado de sesiones guardado: graficas_TFG/{loc}/{ruta_img.name}")
...
def generar_scatter_medias_localizaciones():
    ...
    try:
        # 1. Calculamos medias y medianas
        ...
        # Aseguramos que estén ordenadas igual para poder unir los puntos luego en la combinada
        ...
        # Sub-función para dibujar y no repetir código
        def dibujar_scatter(tipo, titulo, archivo):
            fig, ax = plt.subplots(figsize=(10, 10))

            # Dibujamos MEDIAS (Naranja, Círculo)
            if tipo in ['mean', 'combined']:
                ...
                # Ponemos los textos de las plazas siempre anclados a la media
                for idx, row in df_medias.iterrows():
                    ax.annotate(str(row[col_loc]), (row[col_iso_p], row[col_iso_e]),
                                xytext=(8, 5), textcoords="offset points",
                                fontsize=10, fontweight="bold", color="#1a1a1a")

            # Dibujamos MEDIANAS (Verde, Cuadrado en la combinada para distinguirlo mejor)
            if tipo in ['median', 'combined']:
                ...
                # Ponemos los textos a la mediana SOLO si es la gráfica individual de medianas
                if tipo == 'median':
            ...
            # Detalles exclusivos de la gráfica COMBINADA
            if tipo == 'combined':
                # Unimos la media y la mediana de la misma plaza con una línea punteada gris
                ...
                # Colocamos la leyenda fuera para no tapar puntos
                ax.legend(loc='upper left', bbox_to_anchor=(1.05, 1), fontsize=12, markerscale=1.5)

            # Aplicamos formato ISO y guardamos
        ...
        # 2. Generamos las 3 gráficas usando la sub-función
...
def generar_jointplot_localizacion():
    ...
    while True:
        ...
        for loc in locs_a_procesar:
            ...
            try:
                # Usamos JointGrid de seaborn para controlar perfectamente el centro y los márgenes
                g = sns.JointGrid(data=df_loc, x=col_iso_p, y=col_iso_e, height=8, space=0.2)

                # 1. PINTAMOS EL CENTRO: Nube de Densidad + Puntos (igual que la Opción 4)
                ...
                # 2. PINTAMOS LOS MÁRGENES: Histogramas
                ...
                # 3. APLICAMOS LA CUADRÍCULA ISO: (Pasamos título vacío para ponerlo arriba globalmente)
                dar_formato_ejes_iso(g.ax_joint, "")
                
                # Título general de la figura ajustado arriba
                ...
                # plt.close() no es estrictamente necesario con g.savefig, pero ayuda a limpiar la memoria
                plt.close(g.fig) 
                
                print(f"✅ Joint Plot guardado: graficas_TFG/{loc}/{ruta_img.name}")
...
def generar_jointplot_agrupado_localizaciones():
    ...
    while True:
        ...
        if entrada == '0':
            ...
            if len(locs_elegidas) == 0:
                ...
                break # Salimos del bucle para dibujar la gráfica
        else:
    ...
    # Filtramos la base de datos para quedarnos solo con las plazas que ha metido el usuario
    df_filtrado = df_datos[df_datos[col_loc].isin(locs_elegidas)]

    try:
        # 1. Activamos el fondo gris claro (darkgrid) SÓLO para esta gráfica
        sns.set_style("darkgrid") 
        
        # Usamos paleta 'bright' para maximizar el contraste entre plazas
        paleta = sns.color_palette("bright", len(locs_elegidas))
        
        # 2. Creamos el JointGrid cambiando space=0.2 por space=0 (para pegarlo todo)
        g = sns.JointGrid(data=df_filtrado, x=col_iso_p, y=col_iso_e, hue=col_loc, palette=paleta, height=8, space=0)

        # 1. PINTAMOS EL CENTRO
        # Puntos de dispersión (¡Activamos la leyenda AQUÍ para que salgan circulitos claros!)
        ...
        # Nube interior transparente (Leyenda APAGADA)
        ...
        # Contorno exterior fuerte (Leyenda APAGADA)
        sns.kdeplot(data=df_filtrado, x=col_iso_p, y=col_iso_e, hue=col_loc, palette=paleta, 
                    fill=False, linewidths=2, levels=[0.1], ax=g.ax_joint, legend=False)

        # 2. PINTAMOS LOS MÁRGENES CON CURVAS KDE
        ...
        # 3. FORMATO ISO Y LEYENDA
        dar_formato_ejes_iso(g.ax_joint, "")
        g.fig.suptitle("Comparative Joint Plot", y=1.02, fontsize=16)
        
        # Colocamos la leyenda FUERA de la gráfica (a la derecha) para que no tape los datos
        ...
        print(f"✅ Joint Plot comparativo guardado: graficas_TFG/General/{ruta_img.name}")
...
def resumir_datos_localizaciones():
    ...
    # Buscamos las columnas de coordenadas ISO y los 8 PAQs
    ...
    # --- 1. CÁLCULO DE TABLAS (MEDIAS Y MEDIANAS) ---
    ...
    while True:
        ...
        if sub_op == '1':
            carpeta = obtener_carpeta_guardado("General")
            ruta_csv = carpeta / "11A_Resumen_Subjetivo_Medias.csv"
            df_resumen_medias.to_csv(ruta_csv, index=False, sep=';', decimal=',')
            print(f"✅ ¡CSV de MEDIAS guardado con éxito en: graficas_TFG/General/{ruta_csv.name}!")
            
        elif sub_op == '2':
            carpeta = obtener_carpeta_guardado("General")
            ruta_csv = carpeta / "11B_Resumen_Subjetivo_Medianas.csv"
            df_resumen_medianas.to_csv(ruta_csv, index=False, sep=';', decimal=',')
            print(f"✅ ¡CSV de MEDIANAS guardado con éxito en: graficas_TFG/General/{ruta_csv.name}!")
            ...
            while True:
                ...
                if match:
                    # Extraemos las filas de ambas tablas
...
def resumir_caracteristicas_acusticas():
    ...
    # 1. Filtro ULTRA-ROBUSTO para capturar todo sin que se escape nada
    ...
    for c in cols:
        cl = str(c).lower().strip() # Lo pasamos a minúscula y quitamos espacios
        if cl in excluir:
            continue
        ...
        es_psico = False
        for p in psico_base:
            # Captura 'n', 'n_avg', 'n5', etc., pero no 'name' ni 'notes'
    ...
    # 2. Conversión forzosa a números (Salva las columnas si vienen con comas en vez de puntos)
    cols_acusticas_validas = []
    for col in cols_acusticas_brutas:
        # Si es texto (object), cambiamos comas por puntos
        if df_datos[col].dtype == 'object':
            df_datos[col] = df_datos[col].astype(str).str.replace(',', '.', regex=False)
        
        # Forzamos la conversión a numérico
        df_datos[col] = pd.to_numeric(df_datos[col], errors='coerce')
        
        # Si la columna tiene al menos un número válido, la incluimos
    ...
    if not cols_acusticas_validas:
        ...
        print("Revisa tu Excel original para ver cómo se llaman las columnas de ruido.")
        return

    # --- Preparamos los DataFrames a exportar ---
    ...
    # DF 1: CSV SOLO Acústica (Filas crudas)
    df_completo_acustico = df_datos[cols_identificadoras + cols_acusticas_validas]

    # DF 2 y 3: Resumen Medias y Medianas por Localización
    df_count = df_datos.groupby(col_loc).size().reset_index(name='count')
    ...
    while True:
        ...
        if sub_op == '1':
            carpeta = obtener_carpeta_guardado("General")
            ruta_csv_1 = carpeta / "12A_Datos_Acusticos_Filas_Crudas.csv"
            df_completo_acustico.to_csv(ruta_csv_1, index=False, sep=';', decimal=',')
            print(f"✅ ¡CSV ACÚSTICO (todas las filas) guardado en: graficas_TFG/General/{ruta_csv_1.name}!")
            
        elif sub_op == '2':
            carpeta = obtener_carpeta_guardado("General")
            ruta_csv_2 = carpeta / "12B_Resumen_Medias_Acusticas.csv"
            df_resumen_medias.to_csv(ruta_csv_2, index=False, sep=';', decimal=',')
            print(f"✅ ¡CSV de MEDIAS guardado con éxito en: graficas_TFG/General/{ruta_csv_2.name}!")
            
        elif sub_op == '3':
            carpeta = obtener_carpeta_guardado("General")
            ruta_csv_3 = carpeta / "12C_Resumen_Medianas_Acusticas.csv"
            df_resumen_medianas.to_csv(ruta_csv_3, index=False, sep=';', decimal=',')
            print(f"✅ ¡CSV de MEDIANAS guardado con éxito en: graficas_TFG/General/{ruta_csv_3.name}!")
...
def generar_csv_machine_learning_global():
    ...
    try:
        # Hacemos una copia profunda del dataframe original limpio para no alterar el resto del programa
        df_ml = df_datos.copy()
        
        # Comprobamos si las coordenadas ISO ya están calculadas. Si no, las calculamos.
        # Ajustamos a minúsculas para buscar fácilmente
        ...
        if 'isopleasant' not in columnas_actuales or 'isoeventful' not in columnas_actuales:
            ...
            print("✅ Coordenadas ISO ya detectadas en el dataset.")

        # Aseguramos que la carpeta destino exista
        # Asumiendo que 'carpeta_graficas_base' está definida al principio de tu script
        carpeta_ml = carpeta_graficas_base / 'General'
        carpeta_ml.mkdir(parents=True, exist_ok=True)
        
        ruta_guardado = carpeta_ml / "ML_Datos_Globales.csv"
        
        # Guardamos el CSV Maestro
        df_ml.to_csv(ruta_guardado, index=False, sep=';', decimal=',')
        
        print("\n" + "-"*40)
        print(f"✅ ¡ÉXITO! Base de datos para la Inteligencia Artificial creada.")
        print(f"📊 Filas totales (Encuestas válidas): {len(df_ml)}")
...
def generar_histograma_ciudades():
    ...
    agrupa las localizaciones por ciudad y genera un gráfico de barras.
    """
    global df_datos # Usamos el df global que tu script ya cargó al principio
    
    print("\n" + "📊"*20)
    print(" GENERANDO HISTOGRAMA DE DISTRIBUCIÓN POR CIUDADES ")
    print("📊"*20)
    
    try:
        # Hacemos una copia para no romper el df original
        ...
        mapeo_ciudades = {
            ...
            'miradorsannicolas': 'Granada',
            'plazabibrambla': 'Granada',
            'monumentogaribaldi': 'Venecia',
            'sanmarco': 'Venecia',
        ...
        conteo = df_plot['ciudad'].value_counts().reset_index()
        conteo.columns = ['Ciudad', 'Número de Encuestas']
        
        # 3. Dibujar la Gráfica
        plt.figure(figsize=(10, 6))
        
        # Colores: Destacamos Groningen (Test del ML) en rojo, el resto en azul
        colores = ['#e74c3c' if c == 'Groningen' else '#3498db' for c in conteo['Ciudad']]
        
        ax = sns.barplot(data=conteo, x='Ciudad', y='Número de Encuestas', 
                         palette=colores, edgecolor='black', linewidth=1.2)
        
        # Estética
        plt.title('Distribución de la Muestra por Ciudades', fontsize=16, fontweight='bold', pad=15)
        plt.xlabel('Ciudad de Origen', fontsize=12, fontweight='bold')
        plt.ylabel('Volumen de Encuestas (N)', fontsize=12, fontweight='bold')
        plt.grid(axis='y', linestyle='--', alpha=0.7)
        
        # Números exactos encima de las barras
        for container in ax.containers:
            ax.bar_label(container, fmt='%d', padding=5, fontsize=12, fontweight='bold')
            
        # Leyenda explicativa
        import matplotlib.patches as mpatches
        train_patch = mpatches.Patch(color='#3498db', label='Datos de Entrenamiento (ML)')
        test_patch = mpatches.Patch(color='#e74c3c', label='Groningen (Datos de Test ML)')
        plt.legend(handles=[train_patch, test_patch], loc='upper right', framealpha=0.9)

        # Ajuste de ejes para que quepan los números y la leyenda
        y_max = conteo['Número de Encuestas'].max()
        ...
        plt.savefig(ruta_img, bbox_inches='tight', dpi=300, facecolor='white')
        print(f"✅ Gráfica guardada con éxito en: graficas_TFG/General/{ruta_img.name}")
        plt.show()
        
    except Exception as e:
        print(f"❌ Ocurrió un error al generar el histograma: {e}")
...
if __name__ == "__main__":
    while True:
        print("\n" + "*"*40)
        print(" MENÚ DE ANÁLISIS TFG SOUNDSCAPE ")
        print("*"*40)
        ...
        print("8. Generar Scatter Plot de Medias ISO General")
        print("9. Generar Joint Plot con Histogramas por Localización")
        print("10. Generar Joint Plot Agrupado (Comparativa a la carta)")
        print("11. Ver Tabla Resumen de Variables Subjetivas (PAQ --> medias) y Guardar CSV")
        print("12. Extraer/Consultar Datos Acústicos Objetivos (Medias y Crudos)")
        print("13. 🤖 Generar CSV Maestro para Machine Learning (Todas las variables + ISO)")
        print("14. 📊 Generar Histograma de Muestra por Ciudades (General)")
        print("0. Salir")
        
        opcion = input("\nElige una opción (0-14): ").strip()
        ...
        if opcion == '1':
            ...
            generar_csv_machine_learning_global()
        elif opcion == '14':
            generar_histograma_ciudades()
        elif opcion == '0':
            print("Saliendo del programa...")
            print("¡Adiós!")
            break
        else:
            print("❌ Opción no válida. Por favor, elige un número del 0 al 14.")