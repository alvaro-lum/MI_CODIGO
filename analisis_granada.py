...
# Definimos la NUEVA carpeta principal de gráficas de forma GLOBAL para Granada
carpeta_graficas_base = ruta_script / 'Graficas_Granada_TFG'
...
try:
    ...
    # 6. FILTRO GEOGRÁFICO: Solo localizaciones de Granada
    localizaciones_granada = [
        'CampoPrincipe', 'CarloV', 'MiradorSanNicolas', 'PlazaBibRambla'
    ]
    
    # Nos aseguramos de ignorar mayúsculas/minúsculas al filtrar
    localizaciones_granada_lower = [loc.lower() for loc in localizaciones_granada]
    
    if 'locationid' in df_datos.columns:
        # Filtramos quedándonos solo con las filas cuyo locationid esté en la lista de Granada
        df_datos = df_datos[df_datos['locationid'].str.lower().isin(localizaciones_granada_lower)]
    else:
        print("⚠️ Advertencia: No se encontró la columna 'locationid'. No se pudo aplicar el filtro de Granada.")

    # 7. Resumen de la depuración
    filas_limpias = df_datos.shape[0]
    filas_eliminadas = filas_originales - filas_limpias
    
    print(f"🧹 DEPURACIÓN ESTRICTA: Se han eliminado {filas_eliminadas} filas (incompletas, duplicadas o fuera de Granada).")
    print(f"✅ Datos listos para el análisis: {filas_limpias} participantes únicos en plazas de Granada.")
    ...
    ruta_exportacion = carpeta_graficas_base / "ISD_Datos_Granada_Limpios_TFG.csv"
...
def obtener_carpeta_guardado(localizacion="General"):
    """Crea y devuelve la ruta de una subcarpeta dentro de Graficas_Granada_TFG."""
...
def resumir_y_exportar_ISO():
    ...
    ruta_exportacion = carpeta_graficas_base / "ISD_Parametros_ISO_Granada_TFG.csv"
...
def generar_radar_circunflejo_paq_global():
    ...
    def dibujar_radar(lista_datos, lista_colores, lista_etiquetas, titulo, nombre_archivo):
        ...
        plt.close() 
        print(f"  ✅ Guardado: Graficas_Granada_TFG/General/{ruta_img.name}")
...
def generar_radar_localizacion_especifica():
    ...
    for loc in locs_a_procesar:
        ...
        print(f"✅ 3 Gráficos guardados para {loc} en Graficas_Granada_TFG/{loc}")
...
def generar_iso_plot_combinado():
    ...
    for loc in locs_a_procesar:
        ...
        try:
            ...
            print(f"✅ Gráfico combinado guardado: Graficas_Granada_TFG/{loc}/{ruta_img.name}")
        ...
        for loc in locs_a_procesar:
            try:
                ...
                if respuesta_grafica == 's':
                    ...
                    plt.savefig(ruta_img, bbox_inches='tight', dpi=300, facecolor='white')
                    plt.close()
                    print(f"✅ Gráfica comparativa SPI guardada: Graficas_Granada_TFG/{loc}/{ruta_img.name}\n")
                else:
                ...
                if respuesta_superpuesta == 's':
                    ...
                    plt.close()
                    print(f"✅ Gráfica superpuesta guardada: Graficas_Granada_TFG/{loc}/{ruta_img_super.name}\n")
                else:
...
def generar_scatter_agrupado_localizacion():
    ...
    try:
        ...
        plt.savefig(ruta_img, bbox_inches='tight', dpi=300, facecolor='white')
        plt.close()
        print(f"✅ Gráfico scatter agrupado guardado: Graficas_Granada_TFG/General/{ruta_img.name}")
    ...
    while True:
        ...
        for loc in locs_a_procesar:
            ...
            try:
                ...
                plt.savefig(ruta_img, bbox_inches='tight', dpi=300, facecolor='white')
                plt.close()
                print(f"  ✅ Gráfico agrupado de sesiones guardado: Graficas_Granada_TFG/{loc}/{ruta_img.name}")
    ...
    while True:
        ...
        for loc in locs_a_procesar:
            ...
            try:
                ...
                print(f"✅ Joint Plot guardado: Graficas_Granada_TFG/{loc}/{ruta_img.name}")
...
def generar_jointplot_agrupado_localizaciones():
    ...
    try:
        ...
        print(f"✅ Joint Plot comparativo guardado: Graficas_Granada_TFG/General/{ruta_img.name}")
...
def resumir_datos_localizaciones():
    ...
    while True:
        ...
        if sub_op == '1':
            carpeta = obtener_carpeta_guardado("General")
            ruta_csv = carpeta / "11A_Resumen_Subjetivo_Granada_Medias.csv"
            df_resumen_medias.to_csv(ruta_csv, index=False, sep=';', decimal=',')
            print(f"✅ ¡CSV de MEDIAS guardado con éxito en: Graficas_Granada_TFG/General/{ruta_csv.name}!")
            
        elif sub_op == '2':
            carpeta = obtener_carpeta_guardado("General")
            ruta_csv = carpeta / "11B_Resumen_Subjetivo_Granada_Medianas.csv"
            df_resumen_medianas.to_csv(ruta_csv, index=False, sep=';', decimal=',')
            print(f"✅ ¡CSV de MEDIANAS guardado con éxito en: Graficas_Granada_TFG/General/{ruta_csv.name}!")
...
def resumir_caracteristicas_acusticas():
    ...
    while True:
        ...
        if sub_op == '1':
            carpeta = obtener_carpeta_guardado("General")
            ruta_csv_1 = carpeta / "12A_Datos_Acusticos_Granada_Filas_Crudas.csv"
            df_completo_acustico.to_csv(ruta_csv_1, index=False, sep=';', decimal=',')
            print(f"✅ ¡CSV ACÚSTICO (todas las filas) guardado en: Graficas_Granada_TFG/General/{ruta_csv_1.name}!")
            
        elif sub_op == '2':
            carpeta = obtener_carpeta_guardado("General")
            ruta_csv_2 = carpeta / "12B_Resumen_Medias_Granada_Acusticas.csv"
            df_resumen_medias.to_csv(ruta_csv_2, index=False, sep=';', decimal=',')
            print(f"✅ ¡CSV de MEDIAS guardado con éxito en: Graficas_Granada_TFG/General/{ruta_csv_2.name}!")
            
        elif sub_op == '3':
            carpeta = obtener_carpeta_guardado("General")
            ruta_csv_3 = carpeta / "12C_Resumen_Medianas_Granada_Acusticas.csv"
            df_resumen_medianas.to_csv(ruta_csv_3, index=False, sep=';', decimal=',')
            print(f"✅ ¡CSV de MEDIANAS guardado con éxito en: Graficas_Granada_TFG/General/{ruta_csv_3.name}!")
...
def generar_csv_machine_learning_granada():
    """Opción 13: Genera y guarda el CSV Maestro de Granada con todos los datos y coordenadas ISO para la IA."""
    print("\n" + "="*60)
    print(" 🤖🎡 GENERANDO CSV MAESTRO PARA MACHINE LEARNING (GRANADA) 🎡🤖 ")
    print("="*60)
    
    try:
        # Hacemos una copia profunda del dataframe original limpio (que ya es solo de Granada)
        df_ml = df_datos.copy()
        ...
        ruta_guardado = carpeta_ml / "ML_Datos_Granada.csv"
        
        # Guardamos el CSV Maestro
        df_ml.to_csv(ruta_guardado, index=False, sep=';', decimal=',')
        
        print("\n" + "-"*40)
        print(f"✅ ¡ÉXITO! Base de datos de GRANADA lista para la Inteligencia Artificial.")
        print(f"📊 Filas totales (Encuestas válidas en Granada): {len(df_ml)}")
...
if __name__ == "__main__":
    while True:
        ...
        print("13. 🤖 Generar CSV Maestro para Machine Learning (GRANADA)")
        print("0. Salir")
