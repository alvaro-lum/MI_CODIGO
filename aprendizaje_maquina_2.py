# ==========================================
# Codigo 2: aprendizaje máquina general sin distinción de grupos.
...
# === MI ESTRATEGIA DE EXAMEN (TEST ESPACIAL) ===
# Para demostrar que mi modelo funciona de verdad, entreno con 3 ciudades 
# y me guardo Groningen como "examen sorpresa" que el modelo nunca ha visto.
ciudad_test = 'Groningen'

if ciudad_test in dfs_ciudades:
    print(f"🎯 Variables objetivo configuradas: ISOPLEASANT e ISOEVENTFUL")
    
    # Saco los datos del examen sorpresa
    df_test_crudo = dfs_ciudades[ciudad_test].copy()
    
    # Junto los datos de las otras 3 ciudades en una sola súper-tabla para entrenar
    ...
    # Aplico mi función de limpieza a ambos sets
...
def evaluar_ciudad_completa(mejores_modelos, X_test, y_test_real, target_name):
    ...
    carpeta_resultados = ruta_script / 'Graficas_IA_M2_TFG' / 'Resultados_Metricas'
...
def generar_auditoria_individual(mejores_modelos, X_train, y_train, X_test, y_test, target_name):
    ...
    carpeta_auditoria = ruta_script / 'Graficas_IA_M2_TFG' / 'Auditorias_Individuales'
...
def generar_comparativa_global_ias(df_resumen_global, target_name):
    ...
    # Guardar la imagen
    carpeta_comparativas = ruta_script / 'Graficas_IA_M2_TFG' / 'Comparativas_Globales'
...
def extraer_e_imprimir_importancias(mejores_modelos, X_train, y_train, target_name):
    ...
    carpeta_pesos = ruta_script / 'Graficas_IA_M2_TFG' / 'Interpretabilidad'
...
def generar_super_prediccion_ensamblada(mejores_modelos, df_ranking_universal, X_test, y_test_real, target_name):
    ...
    plt.tight_layout()
    carpeta_ensamble = ruta_script / 'Graficas_IA_M2_TFG' / 'Ensamblaje'
